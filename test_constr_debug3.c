...
int main() {
    ...
    /* Create a properly allocated string for lookup */
    c8 *none_str = sp_alloc(5);
    memcpy(none_str, "None", 4);
    none_str[4] = '\0';
    sp_str_t none_key = {.data = none_str, .len = 4};
    
    /* Look up None constructor */
    fxsh_constr_info_t *info = sp_ht_getp(constr_env, none_key);
    if (info) {
        printf("None constructor found!\n");
        printf("None type: %s\n", fxsh_type_to_string(info->constr_type));
        printf("None arity: %d\n", info->arity);
    } else {
        printf("None constructor NOT found with allocated key!\n");
    }
