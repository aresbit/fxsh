...
echo "[comptime] examples/comptime_type_ctor.fxsh"
out6=$("$BIN" examples/comptime_type_ctor.fxsh 2>/dev/null)
echo "$out6" | grep -q "comptime VecInt = {data: int; len: int; cap: int}"
echo "$out6" | grep -q "comptime VecStr = {data: string; len: int; cap: int}"
echo "$out6" | grep -q "comptime VecHasData = true"
echo "$out6" | grep -q "comptime VecFields = \\[\"data\", \"len\", \"cap\"\\]"
echo "$out6" | grep -q "comptime IntList = int list"
echo "$out6" | grep -q "comptime StrOption = string option"
echo "$out6" | grep -q "comptime IntResult = int string result"
echo "$out6" | grep -q "comptime ListFields = \\[\\]"
echo "$out6" | grep -q "comptime IntBox = int box"
echo "$out6" | grep -q "comptime Flag = flag"
echo "$out6" | grep -q "comptime UserHasId = true"
echo "$out6" | grep -q "comptime UserFields = \\[\"id\", \"name\"\\]"
echo "$out6" | grep -q "comptime PairFields = \\[\\]"
echo "$out6" | grep -q "comptime UserTypeName = \"{id: int; name: string}\""
echo "$out6" | grep -q "comptime PairTypeName = \"(int, string)\""
echo "$out6" | grep -q "comptime UserIsRecord = true"
echo "$out6" | grep -q "comptime PairIsTuple = true"
echo "$out6" | grep -q "comptime VecIsRecord = true"
